# Spring 框架系列 :id=spring-framework

## 1. Spring 是什么? :id=spring

- 概述

 ?> Spring 是个 java 企业级应用的开源开发框架，是轻量、松耦合的，旨在降低应用程序开发的复杂度。
Spring 框架目标是简化 Java 企业级应用开发，并通过 POJO 为基础的编程模型促进良好的编程习惯。
具有分层体系结构，允许用户选择组件，同时为其它框架的继承提供了很大的便利，可以很容易地集成其它框架（框架中的框架）。
...
## 2. Spring有哪些特性？ :id=feature-spring
...
## 3. Spring 有哪些组成模块？ :id=module-spring
...
### 4.1. 什么是 IOC（控制反转）？ :id=spring-ioc
...
### 4.6. Spring 中依赖注入的几种方式？

- 构造函数注入
- Setter 注入
- 接口注入

### 4.7. 构造器注入和 Setter 注入的区别？
...
### 4.8. BeanFactory 和 ApplicationContext 的区别?
...
### 4.9. ApplicationContext 有哪几种具体实现?
...
### 5.6. 指出在 spring aop 中 concern 和 cross-cutting concern 的不同之处。
...
### 5.8. Spring AOP and AspectJ AOP 有什么区别？

Spring AOP 基于动态代理方式实现；AspectJ 基于静态代理方式实现。Spring
AOP 仅支持方法级别的 PointCut；提供了完全的 AOP 支持，它还支持属性级
别的 PointCut。

### 5.9. 如何理解 Spring 中的代理？

### 5.10. 什么是编织（Weaving）？

## 6. 配置文件

### 6.1. FileSystemResource 和 ClassPathResource 有何区别？
...
### 7.1. Spring 中的 Bean 是什么？

- 是构造用于应用程序的主干对象
- 基于用户提供的配置元信息进行创建
- 由 Spring IOC 容器负责创建、装配、管理

### 7.2. Spring 中的 Bean 的配置方式有哪些？
...
### 7.6. Spring 框架中的单例 Beans 是线程安全的么？
